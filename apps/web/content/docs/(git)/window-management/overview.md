...
- `jump_labels` — Defines the ordered characters used for jump hints when in overview jump mode. Each visible window is assigned a label in this order, and pressing the corresponding key jumps to that window. The number of labels limits how many windows can be assigned hints at once. A label matches the character the active layout produces, and also the key that the configured layouts (`xkb_rules_layout`) assign to that character, with the reference `us` layout as fallback. Jump mode therefore also works while a non-latin layout is active.
